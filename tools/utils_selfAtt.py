...
class SelfAtt(nn.Module):
    ...
    def forward(self, input):
        ...
        query = self.conv_query(input).view(N, self.out_channels, H*W).permute(0, 2, 1).contiguous() # (N, H*W, out_channels)
        key = self.conv_key(input).view(N, self.out_channels, H*W) # (N, -1, H*W)
        value = self.conv_value(input).view(N, self.out_channels, H*W) # (N, -1, H*W)
