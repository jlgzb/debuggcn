...
def seq_translation(skes_joints):
    for idx, ske_joints in enumerate(skes_joints):
        num_frames = ske_joints.shape[0]
        num_bodies = 1 if ske_joints.shape[1] == 75 else 2  # by gzb: the dim of vector, 75dim for one action
        if num_bodies == 2:
            missing_frames_1 = np.where(ske_joints[:, :75].sum(axis=1) == 0)[0]
            missing_frames_2 = np.where(ske_joints[:, 75:].sum(axis=1) == 0)[0]
            cnt1 = len(missing_frames_1) # by gzb: ?
            cnt2 = len(missing_frames_2)

        i = 0  # get the "real" first frame of actor1
        while i < num_frames:
            if np.any(ske_joints[i, :75] != 0):  # by gzb: if there are two actors...
                break
            i += 1

        # by gzb: the second keypoint (neck)
...
def align_frames(skes_joints, frames_cnt):
    ...
    """
    num_skes = len(skes_joints)  # by gzb: the num of samples?
    max_num_frames = frames_cnt.max()  # 300
    aligned_skes_joints = np.zeros((num_skes, max_num_frames, 150), dtype=np.float32) # by gzb: the vector of all samples (videos)?
...
def one_hot_vector(labels):
    ...
    for idx, l in enumerate(labels):  # by gzb: "l" is the int value, such as 59, which means the class number.
...
def split_dataset(skes_joints, label, performer, camera, evaluation, save_path):
    ...
    # by gzb: add in 20211110 from CTR-GCN
...
if __name__ == '__main__':
    ...
    output: NTU_CS.npz, NTU_CV.npz 20211215; ori: NTU_CS.h5, NTU_CV.h5
        CS: cross Subject (Subject IDs); 
        CV: cross view (Camera IDs), 2, 3 for train; 1 for test

    #in CTR-GCN
    output: NTU60_CS.npz, NTU60_CV.npz
    '''
    ...
    label = np.loadtxt(label_file, dtype=np.int64) - 1  # action label: 0~59 # by gzb: all elements subtract 1
